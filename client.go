...
func (b *Talkiepi) TransmitStart() {
	...
	// turn on our transmit LED
        fmt.Println("Yo, I'm talking!")
	var netClient = &http.Client{
 		Timeout: time.Second * 10,
	}
	resp,_ := netClient.Get("http://10.1.1.235:8080/fadeOut")
	resp.Body.Close()
...
func (b *Talkiepi) TransmitStop() {
	...
	fmt.Println("Hey, I just shut up!")
	var netClient = &http.Client{
                Timeout: time.Second * 10,
        }
        resp,_ := netClient.Get("http://10.1.1.235:8080/fadeIn")
        resp.Body.Close()
